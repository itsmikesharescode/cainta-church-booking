<script lang="ts">
  ...
  import { cancelReservationSchema, type CancelReservationSchema } from './schema';
  ...
  interface Props {
    cancelReservationForm: SuperValidated<Infer<CancelReservationSchema>>;
  }
  ...
  const form = superForm(cancelReservationForm, {
    validators: zodClient(cancelReservationSchema),
  ...
  const open = $derived(page.url.searchParams.get('modal') === 'cancel-reservation');
